...
def test_static_server_serves_index(tmp_path: Path) -> None:
    ...
    (static_dir / "styles.css").write_text("body { color: red; }", encoding="utf-8")
...
def test_static_server_serves_styles(tmp_path: Path) -> None:
    static_dir = tmp_path / "static"
    static_dir.mkdir()
    (static_dir / "index.html").write_text("<h1>ok</h1>", encoding="utf-8")
    (static_dir / "app.js").write_text("console.log('ok')", encoding="utf-8")
    ...
    async def run_test() -> None:
        ...
        await asyncio.sleep(0.05)
        try:
            response = await _http_get(9081, "/styles.css")
        finally:
            task.cancel()
            try:
                await task
...
def test_static_server_404(tmp_path: Path) -> None:
    ...
    async def run_test() -> None:
        server = StaticUIHTTPServer(host="127.0.0.1", port=9082, static_dir=static_dir)
        task = asyncio.create_task(server.run())
        await asyncio.sleep(0.05)
        try:
            response = await _http_get(9082, "/missing")
        finally:
            task.cancel()
            try:
                await task
...
def test_repo_static_files_exist() -> None:
    static_dir = Path(__file__).resolve().parents[1] / "src" / "windows_agent" / "static"
